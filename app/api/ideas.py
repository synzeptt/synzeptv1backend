...
def list_ideas(user=Depends(get_current_user), db: Session = Depends(get_db)):
    return db.query(models.Idea).filter(models.Idea.user_id == user.id).order_by(models.Idea.timestamp.desc()).all()
...
def create_idea(idea_in: IdeaCreate, user=Depends(get_current_user), db: Session = Depends(get_db)):
    idea = models.Idea(user_id=user.id, title=idea_in.title, description=idea_in.description)
    db.add(idea)
    db.commit()
    db.refresh(idea)
    return idea
